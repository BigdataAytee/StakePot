...
test.describe('the queued path', () => {
  /*
   * No service worker in this context.
   *
   * The offline shell re-issues the page's GETs from inside the worker, and a
   * request made by a service worker cannot be routed from the page — so the
   * status poll below sailed straight past `page.route` and the hold that makes
   * this test deterministic never happened. Blocking the worker takes the PWA
   * out of the way of a test that is about the trade sheet, not about caching.
   */
  ...
  test('a busy market says "confirming" instead of pretending the trade is done', async ({
    ...
    // The first status poll is held open, which is the other half of what a busy
    // market looks like — and what makes the confirming state observable at all.
    // Left to run at full speed the first poll answers in milliseconds, so the
    // state this test exists to check would flash past between two assertions and
    // the test would be measuring the runner, not the product.
    let firstPoll = true;
    await page.route(
      (url) => url.pathname.endsWith('/status'),
      async (route) => {
        if (firstPoll) {
          firstPoll = false;
