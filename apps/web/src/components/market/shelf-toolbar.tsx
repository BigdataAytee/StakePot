...
export function ShelfToolbar({ shown, total }: { shown: number; total: number }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2.5 pb-1 pt-3.5">
      <span className="text-sm text-text-muted">
        {shown === total ? shown : `${shown} of ${total}`} market{total === 1 ? '' : 's'}
      </span>

      <div className="ml-auto flex gap-1.5" role="group" aria-label="Shelf">
        {SHELVES.map((option) => (
      ...
      <div className="flex gap-1.5" role="group" aria-label="Sort by">
        {SORTS.map((option) => (
