...
| Launch catalogue not opened              | **Open** | You                        | A migrated database has no markets and no staff, so a fresh deployment has nothing to trade. `scripts/deploy/seed-markets.sql` opens six across both shelves and `scripts/deploy/promote-admin.sql` makes the first staff account; read the questions before running the first one                         |
